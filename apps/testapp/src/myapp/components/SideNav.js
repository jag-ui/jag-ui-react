import { Box, Button, Divider, Flex, Icon, Image, Popover, Text } from "jag-ui-react";
    ...
    <>
      ...
      <Popover
        ...
        <Flex flexWrap="wrap" alignContent="flex-start" minHeight="25vh" maxWidth="37vw" color="text" p={3}>
          {options.map((option) => (
            <SideNavDropDownItem key={option.id} to={option.to} icon={option.icon}>
              {option.title}
            </SideNavDropDownItem>
          ))}
        </Flex>
...
export const SideNav = ({ width = "55px", header, footer, topOptions, moreOptions, bottomOptions }) => {
  return (
    <Box
      sx={{
        display: "flex",
      ...
      <Flex flexDirection="column" width="full" flexGrow={1}>
        {header}
        {topOptions && topOptions.map((option) => <SideNavItem key={option.id} {...option} />)}
